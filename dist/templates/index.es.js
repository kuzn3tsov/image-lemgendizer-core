import { p as l } from "../validation-D92jbWMx.js";
import { j as X } from "../validation-D92jbWMx.js";
const F = [
  // Instagram
  {
...
function y(e) {
  return b[e] ? p.ALL.filter(
    (i) => i.platform && i.platform.toLowerCase() === e.toLowerCase()
  ) : (console.warn(`Unknown platform: ${e}`), []);
}
function v(e) {
  return !w.map((a) => a.id).includes(e) && e !== "all" ? (console.warn(`Invalid category: ${e}`), []) : e === "all" ? p.ALL : p.ALL.filter(
    (a) => a.category && a.category.toLowerCase() === e.toLowerCase()
  );
}
function I(e) {
  return p.ALL.find((i) => i.id === e) || null;
}
function N(e, i, a = "min") {
  return p.ALL.filter((s) => {
    if (!s.width || !s.height) return !1;
    const n = l(s.width), o = l(s.height), t = l(e), r = l(i);
    if (a === "flexible")
      return n.isVariable || o.isVariable;
    if (a === "exact" && (n.isVariable || o.isVariable)) {
      if (n.isVariable && o.isVariable)
        return t.isVariable && r.isVariable;
      if (n.isVariable)
        return t.isVariable && o.value === r.value;
      if (o.isVariable)
        return n.value === t.value && r.isVariable;
    }
    if (n.isVariable || o.isVariable || t.isVariable || r.isVariable)
      return !1;
    switch (a) {
      case "exact":
        return n.value === t.value && o.value === r.value;
      case "min":
        return n.value >= t.value && o.value >= r.value;
      case "max":
        return n.value <= t.value && o.value <= r.value;
      case "aspect":
        const c = n.value / o.value, d = t.value / r.value;
        return Math.abs(c - d) < 0.1;
      default:
        return !1;
    }
  });
...
function z(e, i) {
  ...
  if (!e || !e.width || !e.height)
    return 0.5;
  const n = l(i.width), o = l(i.height);
  if (n.isVariable || o.isVariable)
    if (a += 50, n.isVariable && !o.isVariable) {
      const t = e.height / o.value;
      t >= 1 ? a += 30 : t >= 0.8 ? a += 20 * t : a += 10 * t;
      const r = e.width / e.height;
      r >= 0.5 && r <= 2 ? a += 20 : a += 10;
    } else if (!n.isVariable && o.isVariable) {
      const t = e.width / n.value;
      t >= 1 ? a += 30 : t >= 0.8 ? a += 20 * t : a += 10 * t;
      const r = e.width / e.height;
      r >= 0.5 && r <= 2 ? a += 20 : a += 10;
    } else
    ...
    const t = e.width / e.height, r = n.value / o.value, c = Math.abs(t - r) / t;
    a += Math.max(0, 50 * (1 - c));
    const d = e.width / n.value, f = e.height / o.value, m = Math.min(d, f);
    m >= 1 ? a += 30 : a += 30 * m, a += 20;
  }
  return Math.min(s, a) / s;
}
function M(e, i = {}) {
  const {
    category: a = "all",
  ...
  let t = a === "all" ? p.ALL : v(a);
  return s && (t = t.filter((c) => c.platform === s)), o || (t = t.filter((c) => {
    const d = l(c.width), f = l(c.height);
    return !d.isVariable && !f.isVariable;
  })), t.map((c) => {
    const d = z(e, c), f = l(c.width), m = l(c.height);
    return {
      ...c,
      matchScore: d,
      hasFlexibleDimensions: f.isVariable || m.isVariable,
      flexibleType: f.isVariable ? "width" : m.isVariable ? "height" : "none"
    };
  }).filter((c) => c.matchScore >= n).sort((c, d) => d.matchScore - c.matchScore);
}
function L() {
  const e = {
    total: p.ALL.length,
  ...
  return w.forEach((i) => {
    ...
    const a = l(i.width), s = l(i.height);
    if (a.isVariable || s.isVariable)
      e.flexibleTemplates++;
    else {
      e.fixedTemplates++, e.dimensions.minWidth = Math.min(e.dimensions.minWidth, a.value), e.dimensions.maxWidth = Math.max(e.dimensions.maxWidth, a.value), e.dimensions.minHeight = Math.min(e.dimensions.minHeight, s.value), e.dimensions.maxHeight = Math.max(e.dimensions.maxHeight, s.value);
      const n = (a.value / s.value).toFixed(2);
      e.dimensions.commonAspectRatios[n] = (e.dimensions.commonAspectRatios[n] || 0) + 1;
    }
  }), e;
}
function C(e = "json") {
  switch (e.toLowerCase()) {
    ...
    case "csv":
      const i = ["id", "name", "platform", "category", "width", "height", "description", "flexible"], a = p.ALL.map((o) => {
        const t = l(o.width), r = l(o.height), c = t.isVariable || r.isVariable ? "yes" : "no";
        return [
          o.id,
          o.name,
          o.platform,
      ...
      return [i, ...a].map((o) => o.join(",")).join(`
`);
    case "markdown":
      let s = `# LemGendary Templates

`;
      s += `**Total Templates:** ${p.ALL.length}

`;
      const n = L();
      return s += `**Flexible Templates:** ${n.flexibleTemplates}
`, s += `**Fixed Templates:** ${n.fixedTemplates}
...
`;
          const r = {};
          t.forEach((c) => {
            r[c.platform] || (r[c.platform] = []), r[c.platform].push(c);
          }), Object.keys(r).forEach((c) => {
            const d = r[c], f = b[c] || { name: c };
            s += `### ${f.name}
`, d.forEach((m) => {
              const h = l(m.width), g = l(m.height), u = h.isVariable || g.isVariable;
              s += `- **${m.displayName || m.name}**: `, u ? h.isVariable && g.isVariable ? s += "Flexible dimensions" : h.isVariable ? s += `Flexible width × ${g.value}` : s += `${h.value} × Flexible height` : s += `${h.value} × ${g.value}`, m.description && (s += ` - ${m.description}`), u && (s += " ⚡"), s += ` (\`${m.id}\`)
`;
            }), s += `
`;
          });
...
function R(e) {
  const i = [], a = [];
  ["id", "name", "width", "height", "platform", "category"].forEach((t) => {
    e[t] || i.push(`Missing required field: ${t}`);
  }), e.id && typeof e.id != "string" && i.push("id must be a string");
  const n = l(e.width), o = l(e.height);
  return !n.isVariable && (typeof n.value != "number" || n.value <= 0 || isNaN(n.value)) && i.push('width must be a positive number or "flex"'), !o.isVariable && (typeof o.value != "number" || o.value <= 0 || isNaN(o.value)) && i.push('height must be a positive number or "flex"'), e.platform && !b[e.platform] && a.push(`Unknown platform: ${e.platform}`), e.category && !w.find((t) => t.id === e.category) && a.push(`Unknown category: ${e.category}`), e.id && I(e.id) && a.push(`Duplicate template ID: ${e.id}`), e.aspectRatio && (e.aspectRatio === "flexible" || e.aspectRatio === "variable" ? !n.isVariable && !o.isVariable && a.push('aspectRatio is "flexible" but both dimensions are fixed') : e.aspectRatio.match(/^\d+:\d+$/) || a.push(`Invalid aspect ratio format: ${e.aspectRatio}. Use "w:h" or "flexible"`)), {
    valid: i.length === 0,
    errors: i,
    warnings: a,
    hasFlexibleDimensions: n.isVariable || o.isVariable
  };
}
function A(e) {
  const i = R(e);
  if (!i.valid)
    throw new Error(`Invalid template: ${i.errors.join(", ")}`);
  switch (e.category) {
    case "social":
      p.SOCIAL_MEDIA.push(e);
      break;
    case "web":
      p.WEB.push(e);
      break;
    ...
    case "favicon":
      ...
      break;
    default:
      throw new Error(`Invalid category for custom template: ${e.category}`);
  }
  return p.ALL.push(e), {
    template: e,
    validation: i,
...
function W(e) {
  ...
  return p.ALL.filter((a) => a.name && a.name.toLowerCase().includes(i) || a.displayName && a.displayName.toLowerCase().includes(i) || a.description && a.description.toLowerCase().includes(i) || a.platform && a.platform.toLowerCase().includes(i) || a.category && a.category.toLowerCase().includes(i) || a.id && a.id.toLowerCase().includes(i));
}
function E(e) {
  const i = l(e.width), a = l(e.height);
  if (i.isVariable || a.isVariable)
    return "flexible";
  if (!i.value || !a.value) return "unknown";
  const s = (o, t) => t === 0 ? o : s(t, o % t), n = s(i.value, a.value);
  return `${i.value / n}:${a.value / n}`;
}
function O() {
  return p.ALL.filter((e) => {
    const i = l(e.width), a = l(e.height);
    return i.isVariable || a.isVariable;
  });
}
function $(e, i, a = {}) {
  const {
  ...
  let r = s === "all" ? p.ALL : v(s);
  return n && (r = r.filter((d) => d.platform === n)), o || (r = r.filter((d) => {
    const f = l(d.width), m = l(d.height);
    return !f.isVariable && !m.isVariable;
  })), r.map((d) => {
    const f = l(d.width), m = l(d.height);
    let h = 0, g = 1, u = "";
    if (f.isVariable || m.isVariable)
      h = 0.8, f.isVariable && m.isVariable ? (u = "Both dimensions flexible - perfect fit", h = 0.95) : f.isVariable ? (g = i / m.value, g >= 1 ? (h = 0.9, u = "Image height exceeds template height") : g >= t ? (h = 0.8, u = "Image height adequate for template") : (h = 0.5, u = "Image height insufficient for template")) : (g = e / f.value, g >= 1 ? (h = 0.9, u = "Image width exceeds template width") : g >= t ? (h = 0.8, u = "Image width adequate for template") : (h = 0.5, u = "Image width insufficient for template"));
    else {
      const T = e / f.value, B = i / m.value;
      g = Math.min(T, B), g >= 1 ? (h = 1, u = "Image exceeds template dimensions") : g >= t ? (h = 0.7 + g * 0.3, u = "Image adequate for template") : (h = g, u = "Image too small for template");
      const P = e / i, V = f.value / m.value;
      Math.abs(P - V) > 0.1 && (h *= 0.8, u += " (aspect ratio mismatch)");
    }
    return {
      ...d,
      matchScore: h,
  ...
  }).sort((d, f) => f.matchScore - d.matchScore);
}
const D = {
  LemGendTemplates: p,
  TEMPLATE_CATEGORIES: w,
  PLATFORMS: b,
  getTemplatesByPlatform: y,
  getTemplatesByCategory: v,
  getTemplateById: I,
  getTemplatesByDimensions: N,
  getRecommendedTemplates: M,
  getTemplateStats: L,
  exportTemplates: C,
  validateTemplate: R,
  addCustomTemplate: A,
  getAllPlatforms: j,
  getTemplatesGroupedByPlatform: q,
  searchTemplates: W,
  getTemplateAspectRatio: E,
  getFlexibleTemplates: O,
  getTemplatesForImage: $
};
export {
  p as LemGendTemplates,
  b as PLATFORMS,
  w as TEMPLATE_CATEGORIES,
  A as addCustomTemplate,
  D as default,
  C as exportTemplates,
  j as getAllPlatforms,
  O as getFlexibleTemplates,
  M as getRecommendedTemplates,
  E as getTemplateAspectRatio,
  I as getTemplateById,
  L as getTemplateStats,
  v as getTemplatesByCategory,
  N as getTemplatesByDimensions,
  y as getTemplatesByPlatform,
  $ as getTemplatesForImage,
  q as getTemplatesGroupedByPlatform,
  X as isVariableDimension,
  l as parseDimension,
  W as searchTemplates,
  R as validateTemplate
};
