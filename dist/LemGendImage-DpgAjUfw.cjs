...
const index = require("./index-BJHndU-3.cjs");
class LemGendImage {
  /**
  ...
  constructor(file, options = {}) {
    ...
    __publicField(this, "load", async () => {
      try {
        ...
        const analysis = await index.analyzeForOptimization(this.file);
        ...
        if (this.type === "image/png" || this.type === "image/webp" || this.type.includes("icon")) {
          this.transparency = await index.hasTransparency(this.file);
        }
        return this;
      } catch (error) {
    ...
    __publicField(this, "getOutputAsDataURL", async (format) => {
      ...
      return index.fileToDataURL(output.file);
    });
    /**
    ...
    __publicField(this, "toDataURL", async () => {
      return index.fileToDataURL(this.file);
    });
    /**
    ...
    __publicField(this, "createThumbnail", async (maxSize = 200) => {
      return index.createThumbnail(this, maxSize);
    });
    /**
     * Clean up resources
     */
    ...
    this.extension = index.getFileExtension(file);
...
//# sourceMappingURL=LemGendImage-DpgAjUfw.cjs.map
